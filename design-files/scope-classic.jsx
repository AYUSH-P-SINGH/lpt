...
function ScopeClassic({ w = 1280, h = 800 }) {
  ...
  // terrain + coastlines, recomputed each render (cheap; precompute once with useMemo)
  ...
  // selected aircraft
  const [selectedIcao, setSelectedIcao] = React.useState(sim.planes[0].icao);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 18, color: pal.dim }}>
          ...
          <span>UPTIME {fmt.hms(sim.stats.uptimeSec)}</span>
          ...
          <svg width={SCOPE_SIZE - 20} height={SCOPE_SIZE - 20} viewBox={`0 0 ${SCOPE_SIZE - 20} ${SCOPE_SIZE - 20}`}
            ...
            <g transform={`translate(-10 -10)`}>
              ...
              <g clipPath="url(#scope-disk-cls)">
                {/* terrain */}
                <TerrainLayer paths={terrain} color={pal.fg} baseOpacity={0.13} mode="contour" />
                {/* coast & bay */}
                <CoastLayer d={coast} color={pal.fg} opacity={0.35} width={0.9} />
                <CoastLayer d={bay} color={pal.fg} opacity={0.25} width={0.7} />
  ...
  return (
    <div style={{ width: w, height: h, borderLeft: `1px solid ${pal.faint}`,
      ...
      <div style={{ display: 'flex', borderBottom: `1px solid ${pal.faint}` }}>
        <Stat label="MSGS/SEC" value={sim.stats.msgsLastSec.toString().padStart(3, '0')} />
        <Stat label="CRC FAIL" value={`${((sim.stats.crcFailLastSec / Math.max(1, sim.stats.msgsLastSec)) * 100).toFixed(1)}%`} />
        <Stat label="PK SIG" value={`${sim.stats.peakSignalDb.toFixed(1)}`} sub="dBFS" />
      </div>
      ...
      <div style={{ padding: '10px 14px', borderBottom: `1px solid ${pal.faint}` }}>
        ...
        <svg width="100%" height={42} viewBox="0 0 400 42" preserveAspectRatio="none">
          {Array.from({ length: 80 }).map((_, i) => {
            const v = 8 + Math.abs(Math.sin(i * 0.7 + sim.stats.uptimeSec * 2) + Math.cos(i * 0.31)) * 12;
          ...
          {sorted.map((p) => {
            ...
            return (
              <div key={p.icao}
                ...
                <span>{Math.round(p.alt / 100) * 100}</span>
                <span>{Math.round(p.spd)}</span>
                <span>{fmt.hdg(p.hdg)}</span>
      ...
      <div style={{ borderTop: `1px solid ${pal.faint}`, padding: '12px 14px', minHeight: 160 }}>
        ...
        <div style={{ marginTop: 8, fontSize: 9.5, color: pal.dim }}>
          MSGS {selected.msgsRx} · LAST SEEN {((performance.now() - selected.lastSeenMs) / 1000).toFixed(1)}s
        </div>
      </div>
    </div>
  );
